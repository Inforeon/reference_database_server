...
- **CLI path resolution:** User-supplied relative paths are resolved against the current working directory first, then validated as being within the database home. This allows natural usage from subdirectories (e.g., `cd home/proj_1 && docsearch papers add paper.pdf`). Absolute paths must also reside within the database home. The shared helper `resolve_user_path_to_home_relative()` in `cli/utils.py` implements this logic. The `meta` commands use `document_path_candidates()` instead, which yields both the cwd-relative and home-relative interpretations of a relative path so `meta show papers/x.pdf` works from any directory; when no candidate resolves to an indexed document (or a sidecar file) the command fails with a message naming what it tried, rather than printing empty output.
...
- **`models.py`** — `Document` dataclass (indexed document with extracted + sidecar metadata, `document_type`, `source_type`), `Chapter` (textbook chapter with `textbook_id`, `chapter_index`, `title`, `chapter_type`, `start_page`, `end_page`, `page_count`, `file_path`, `metadata`, `full_text`; `combined_metadata()` inherits from parent `Document`), `TextRow` (frozen: a stored extracted-text payload addressed by `kind` (`document`/`chapter`) + `id`, with a human-readable `label`; lets checks treat both tables uniformly), `SearchResult` (document + FTS score + optional `chapter` field + snippet), `SearchQuery` (search parameters: query string, scope, file type, author, tags, date range, `document_types` filter, `raw_fts` flag to pass the query to FTS5 verbatim, pagination). `from_row()` supports `tuple`, `dict`, and `sqlite3.Row` inputs (uses `row.keys()` membership checks for mapping types) and reads metadata columns through `_json_object_or_empty()`, which degrades a malformed column to `{}` rather than raising — one corrupt row must not crash every search that returns it.
- **`repository.py`** — SQLite-backed store with WAL journal mode; **performs no file I/O by design** (the Indexer owns the filesystem side). Uses FTS5 for full-text search over `filename`, `directory`, and `full_text`. Dynamic SQL query builder supporting filters on scope, extension, author, tags, date range, and `document_types`. Query helpers (module-level, shared by `search()` and `search_textbook_chapters()`/`_resolve_textbook_ids()`): `fts_match_query()` compiles free text into an FTS5 expression — each whitespace term becomes a double-quoted string literal (embedded `"` doubled) AND-ed together, so operator characters (`-`, `(`, `:`, `*`, `^`, `NEAR`, bare `OR`) are searched not parsed; a trailing `*` is kept outside the quotes as a prefix operator; no usable term compiles to `'""'` (matches nothing, since FTS5 rejects an empty expression). The raw string reaches MATCH only when `SearchQuery.raw_fts` is set. `_scope_clause()` — `col = ? OR col LIKE ?/'%'`, rstripping trailing slashes and degrading to `1=1` for an empty scope. `_author_clause()` matches by *containment* over the merged metadata view (`_merged_field()` reads sidecar-over-extracted, mirroring `combined_metadata`; `_json_object()` guards malformed columns with `json_valid`) across `author`/`authors`/`authors_bib`, normalising each value to a JSON array (`_as_json_array()`) and requiring every whitespace token of the query within one entry via `instr` (case-insensitive, `%`/`_` literal). Methods: `upsert`, `search`, `get`, `get_by_id`, `remove`, `count`, `all_paths`, `exists`, `list_directory` (filesystem-style directory listing that infers subdirectories from nested document paths), `rename` (update path/filename/directory in-place), `update_document` (selective field update by id), `update_sidecar_metadata` (read-modify-write of the sidecar column inside one transaction, tolerating corrupt JSON; DB half only — writing the file is the Indexer's job), `iter_texts` (streams `TextRow`s from both tables via `fetchmany`). Chapter methods: `upsert_chapter`, `get_chapters`, `get_chapter`, `delete_chapters`, `get_chapter_by_file_path`, `delete_chapter_by_id`, `search_textbook_chapters` (two-phase: resolve textbook IDs from metadata filters → FTS search within those chapters). Migration logic handles adding new columns and renaming existing ones (`_migrate_existing_columns`).
...
- **`handlers.py`** — Pipeline-based document processing framework. Base `DocumentHandler` class with `pre_process()`, `extract_metadata()`, `extract_text()`, `post_process()` hooks. All handlers set `doc.id` from the `upsert` return value so callers get an id without re-querying. Subclasses:
  ...
  - `PaperDocumentHandler` — embeds DOI via `pdf2doi`, runs pdf2bib for bibliographic metadata, strips JATS/HTML markup from registry-derived title/journal/booktitle (`strip_html_markup()`), then corroborates the retrieved title against the file before storing: its own metadata title when present, otherwise page 1 of its text (`_title_appears_in_text()`) — so a lookup that resolved to the wrong record raises instead of being saved silently (interactive terminals prompt to confirm). Moves pdf2bib author list to `authors_bib` key, stores raw bibtex string in sidecar. Falls back to `_generate_bibtex_from_metadata()` when `skip_bib=True`
  ...
  - Helper functions: `strip_html_markup()`, `_normalize_title()` (NFKC + markup/punctuation strip), `_titles_match()`, `_title_appears_in_text()`, `_format_author_dict()`, `_format_authors_bib()`, `_generate_bibtex_from_metadata()`
...
Commands:
  ...
  search                  Search indexed documents (-q, --scope, --type, --author, --tag, --after/--before, --document-types, --raw-fts, --limit, --offset, -f)
  ...
  meta show <FILE>        Display metadata (-k KEY for one field; index first, sidecar file if not indexed)
  ...
  meta init <FILE>        Create empty sidecar file (leaves an existing one untouched)
...
Search supports: `-q QUERY`, `--scope DIR`, `--type EXT`, `--author NAME`, `--tag TAG` (repeatable), `--after/--before DATE`, `--document-types TYPES` (comma-separated), `--raw-fts`, `--limit N`, `--offset N`, `-f FORMAT` (text/json/csv). All output formats include the document `id`.

`-q` is plain text by default: `Repository.fts_match_query()` quotes each term so FTS5 operator characters (`-`, `(`, `:`, `*`, `NEAR`, bare `OR`) are searched rather than parsed (raw `actor-critic` used to raise `no such column: critic`). A trailing `*` still works as a prefix. `--raw-fts` bypasses this and hands the string to FTS5 verbatim. `--author` reads merged metadata (sidecar over extracted) across `author`/`authors`/`authors_bib` and matches by containment, so partial names ("Schulman") and either name in an `"A and B"` string hit; curated sidecar authors win over the PDF's embedded ones.
...
| GET | `/api/search` | Search (query params: q, scope, file_type, author, tags, after, before, document_types, raw_fts, offset, limit). `q` is plain text unless `raw_fts=true` |
...
| `test_search_filters.py` | Query compilation and metadata filters: `fts_match_query()` unit behaviour (quoting/AND, doubled quotes, trailing-`*` prefix vs interior, punctuation-only → empty phrase), operator characters searched not parsed (`actor-critic`, `-plasticity`, `model (based)`) without crashing, prefix search, degenerate queries return nothing, `raw_fts` passes FTS5 syntax through (`OR`) while default treats it as a word; author filter over merged metadata (sidecar list/`"A and B"` string/`authors_bib` dicts, partial surname, name-order forgiven, curated sidecar overrides wrong extracted, corrupt column tolerated) incl. the textbook-chapter path |
| `test_extractors.py` | `PdfExtractor` (metadata extraction, text extraction, multi-page, fault tolerance) |
| `test_handlers.py` | BibTeX helpers (`_normalize_title`, `_titles_match`, `_format_author_dict`, `_format_authors_bib`, `_generate_bibtex_from_metadata`), `strip_html_markup()` (tag strip, entity decode, double-encoded tags, non-string passthrough), title normalisation (markup + NFKC superscript) and `_title_appears_in_text()`, page-1 corroboration in `PaperDocumentHandler.pre_process` (accepts a title present on page 1 when the PDF has no embedded title, rejects an unrelated record, stores a markup-free title), `PaperDocumentHandler` integration (skip_bib, DOI embedding, title mismatch logic, authors_bib handling) |
...
- **FTS queries are plain text by default:** `fts_match_query()` neutralises FTS5 operators so ordinary terms with `-`, `(`, etc. don't crash, which also means interior `*`, column filters (`col:`), `NEAR()`, `^boost` and bare `OR`/`AND` lose their meaning unless passed via `--raw-fts` / `raw_fts=true`. Only a trailing `*` is honoured as a prefix in the default mode
